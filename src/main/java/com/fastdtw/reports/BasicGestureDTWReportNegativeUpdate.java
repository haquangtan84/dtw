...
public class BasicGestureDTWReportNegativeUpdate {
	...
	public static double[][] testByUser(ArrayList<String[]> arrSamples) {
		...
		for(int i = 0; i< arrSamples.size(); i++) {
			if(i==0)  {
			  templates = arrSamples.get(0);
			}
			if(i == arrSamples.size()-1) {
			  y = i;
			} else y = i+1;
			String[] newTemplates = arrSamples.get(y);
			...
			for(int g = 0; g<8; g++) {
				...
				{					
                   ...
                   isReplace = true;
					...
					if(isReplace) {
					  templates[match] = newTemplates[match];
					}
				}		
